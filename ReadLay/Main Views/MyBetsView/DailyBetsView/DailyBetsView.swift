...
struct DailyBetsView: View {
    ...
    var body: some View {
        ...
        // FIXED: Changed from combineLatest to onChange since these aren't Publishers
        .onAppear {
            dailyBetsViewModel.updateDailyBetsWithMultiDay(from: readSlipViewModel.placedBets, readSlipViewModel: readSlipViewModel)
        }
        .onChange(of: readSlipViewModel.placedBets) { _ in
            dailyBetsViewModel.updateDailyBetsWithMultiDay(from: readSlipViewModel.placedBets, readSlipViewModel: readSlipViewModel)
        }
        .onChange(of: readSlipViewModel.betProgress) { _ in
            dailyBetsViewModel.updateDailyBetsWithMultiDay(from: readSlipViewModel.placedBets, readSlipViewModel: readSlipViewModel)
        }
    ...
    private var overlaysView: some View {
        Group {
            startingPageInputOverlay
            continueReadingOverlay
            readingTimerOverlay
            endingPageInputOverlay
            commentInputOverlay
        }
    }

    // Starting Page Input Overlay (First Read)
    @ViewBuilder
    private var startingPageInputOverlay: some View {
        if sessionViewModel.showingStartPageInput,
           ...
           let bet = dailyBetsViewModel.dailyBets.first(where: { $0.betId == session.betId }) {
            ...
            StartingPageInputView(
                ...
                onStart: { startingPage in
                    sessionViewModel.setStartingPage(startingPage)
                }
            )
            .transition(.opacity)
        }
    ...
    @ViewBuilder
    private var continueReadingOverlay: some View {
    ...
    @ViewBuilder
    private var readingTimerOverlay: some View {
    ...
    @ViewBuilder
    private var endingPageInputOverlay: some View {
